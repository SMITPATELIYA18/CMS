<?php
	...
	$emp_id=$row['e_id'];
	//echo $emp_id;
	$sql="select * from delivery_id where e_id='$emp_id'";
	$result=mysqli_query($conn,$sql);
	$row=mysqli_fetch_assoc($result);
	$b_id=$row['branch_id'];
	//echo $b_id;
	...
	$abc=serialize($o_d);
	echo $ccity;
	$sql="update consignment set track='$abc', current='', delivery_id=NULL where con_id=$id";
